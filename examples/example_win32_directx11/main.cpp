...
// ReloBOT state
float relobot_x = 0.0f;
float relobot_y = 0.0f;
float relobot_rotation = 0.0f;
float relobot_zoom = 1.0f;
bool relobot_grid = true;
bool relobot_indicators = true;
    ...
    while (!done)
    {
        ...
        if (isIntro)
            ...
            ImGui::Begin("menu by mon4ik", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize);
            {
                ...
                if (active_tab == 7) {
                    ImGui::SetCursorPos(ImVec2(148, 50));
                    ImGui::BeginChild("RELOBOT_CANVAS", ImVec2(464, 402));
                    {
                        ImGui::PushFont(menu.generalTable);
                        
                        // Canvas area
                        ImVec2 canvas_pos = ImGui::GetCursorScreenPos();
                        ImVec2 canvas_size = ImVec2(444, 300);
                        ImDrawList* draw_list = ImGui::GetWindowDrawList();
                        
                        // Draw canvas background
                        draw_list->AddRectFilled(canvas_pos, ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), 
                                                ImColor(20, 23, 22), 5.0f);
                        
                        // Draw grid if enabled
                        if (relobot_grid) {
                            float grid_step = 20.0f * relobot_zoom;
                        ...
                        // Draw ReloBOT
                        ImVec2 bot_pos = ImVec2(canvas_pos.x + canvas_size.x/2 + relobot_x * relobot_zoom,
                                                canvas_pos.y + canvas_size.y/2 + relobot_y * relobot_zoom);
                        draw_list->AddCircleFilled(bot_pos, 10.0f * relobot_zoom, ImColor(139, 130, 159));
                        
                        // Draw direction indicator
                        float dir_x = cosf(relobot_rotation * 3.14159f / 180.0f) * 20.0f * relobot_zoom;
                        float dir_y = sinf(relobot_rotation * 3.14159f / 180.0f) * 20.0f * relobot_zoom;
                        draw_list->AddLine(bot_pos, ImVec2(bot_pos.x + dir_x, bot_pos.y + dir_y),
                                        ImColor(255, 255, 255));
                        ...
                        ImGui::DragFloat("X Position", &relobot_x, 1.0f, -200.0f, 200.0f);
                        ImGui::SameLine();
                        ImGui::DragFloat("Y Position", &relobot_y, 1.0f, -200.0f, 200.0f);
                        
                        ImGui::DragFloat("Rotation", &relobot_rotation, 1.0f, 0.0f, 360.0f);
                        ImGui::SameLine();
                        ImGui::DragFloat("Zoom", &relobot_zoom, 0.1f, 0.1f, 5.0f);
                        
                        ImGui::Checkbox("Show Grid", &relobot_grid);
                        ImGui::SameLine();
                        ImGui::Checkbox("Show Indicators", &relobot_indicators);
